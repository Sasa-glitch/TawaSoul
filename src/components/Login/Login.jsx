...
export default function Register() {
    ...
    return (
        <>
            {logedIn && <div className="bg-green-700 text-white p-4 rounded-3xl fixed top-0">You Have Been Logged In</div>}
            {errorInSign && <div className="bg-red-700 text-white p-4 rounded-3xl fixed top-0">{errorInSign}</div>}
                <img src={icon} alt="tawasoul app icon" className="w-20 lg:w-30 m-auto"/>
                ...
                <p className="uppercase text-xs text-center mb-2">The Digital Soul of the Ummah</p>
                <div className="container gap-0 m-auto glass-back rounded-4xl overflow-clip w-full max-w-xl">
                    <div className="dark:bg-linear-45 dark:from-oud-wood dark:via-desert-dark-sand dark:to-oud-wood bg-white p-8 md:p-12 lg:p-16 flex flex-col justify-center">
                        ...
                        {logedIn && <div className="bg-green-700 text-white p-4 rounded-3xl">You Have Been Signed Up</div>}
                        ...
                        >
                            ...
                            <div className="flex items-center space-x-2 ml-1 mb-2">
                                <input defaultChecked={true} className="input-select" id="remember" type="checkbox" />
                                <label className="text-xs" htmlFor="remember">Keep me logged in on this device</label>
                            </div>
                            {/* submit button */}
                            ...
                            {/* to login link */}
                            <Link to={"/register"} className="text-primary">Aren't A Memeber Yet? Sign Up</Link>
                        </form> 
                    </div>
                </div>
        </>
